...
app.post('/todos/', async (req, res) => {
  ...
  if (datefns.isValid(dueDate)) {
    dueDate = datefns.format(dueDate, 'yyyy-MM-dd')
  }

  if (statusValid && priorityValid && categoryValid && dateValid) {
    await db.run(
      `insert into todo values (${req.body.id}, '${req.body.todo}', '${req.body.priority}', '${req.body.status}', '${req.body.category}', '${dueDate}')`,
    )
